...
class Trainer():
    ...
    def train(self):
        """
        增加 Early Stopping 支持：验证集 loss 多次未提升则提前终止训练。
        """
        ...
        for epoch in range(self.epoch):
            ...
            for batch_idx, batch in enumerate(self.train_dataloader):
                ...
                if batch_idx % self.log_interval == 0:
                    ...
                    print('\n Train set: loss_t: {:.4f}, loss_a: {:.4f}, loss_v: {:.4f}, loss_m: {:.4f}, loss: {:.4f}'.format(
                        loss_t.item(), loss_a.item(), loss_v.item(), loss_m.item(), loss.item()))
                    # 计算当前batch的准确率
                    pred_t = output[0].argmax(dim=1)
                    pred_a = output[1].argmax(dim=1)
                    pred_v = output[2].argmax(dim=1)
                    pred_m = output[3].argmax(dim=1)
                    acc_t = (pred_t == label_t).float().mean().item()
                    acc_a = (pred_a == label_a).float().mean().item()
                    acc_v = (pred_v == label_v).float().mean().item()
                    acc_m = (pred_m == label_m).float().mean().item()
                    print('Train set: Acc_t: {:.4f}, Acc_a: {:.4f}, Acc_v: {:.4f}, Acc_m: {:.4f}\n'.format(
                        acc_t, acc_a, acc_v, acc_m))
    ...
    def test(self, dataloader):
        ...
        c_r = classification_report(all_label_m, predicted, target_names = labels_eng, digits = 4, zero_division=0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--patience', default=5, type=int, help='Early stopping patience (default: 5)')
